...
function animateCounter(counterElement) {
    ...
    let current = 0;
    let increment = target / 100; // Kecepatan animasi
    ...
    function updateCounter() {
        if (current < target) {
            ...
            // Tunggu sebentar, lalu ulang animasi
            setTimeout(() => startCounter(counterElement), 2000); // Jeda 2 detik sebelum ulang
        }
    }
    updateCounter();
}
...
function startCounter(counterElement) {
    counterElement.innerText = "0"; // Reset angka ke 0
    animateCounter(counterElement); // Mulai animasi lagi
}

document.addEventListener("DOMContentLoaded", function () {
    let counters = document.querySelectorAll(".counter");
    counters.forEach(counter => startCounter(counter)); // Terapkan ke semua elemen
});
